...
async function loadHeroes() {
    try {
        // Use API that returns Cloudinary URLs if configured
        const response = await fetch('/api/herolist');
        if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
        return await response.json();
    } catch (error) {
        console.error('Error loading herolist:', error);
        // Fallback to local file
        try {
            const fallback = await fetch('/database/herolist.json');
